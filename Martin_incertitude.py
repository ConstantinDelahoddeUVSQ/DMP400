...
class ParticleApp:
    ...
    def create_incertitude_widgets(self, parent):
        """
        Widgets pour l'incertitude'
        ...
        self.dynamic_incertitude_inputs_frame = ttk.Frame(frame)
        self.base_incertitude_inputs_frame = ttk.Frame(frame)

        # --- Checkbox "Tracer dynamiquement (électrique)" ---
        self.dynamic_incertitude_var = tk.BooleanVar(value=False)
        dynamic_incertitude_check = ttk.Checkbutton(frame, text="Tracer dynamiquement (électrique)", variable=self.dynamic_incertitude_var, command=self.toggle_dynamic_incertitude)
        dynamic_incertitude_check.pack(anchor=tk.W, pady=5)

        # Vitesse initiale
        self.v0_incertitude_var = tk.StringVar(value="1e6")
        self.add_labeled_entry(frame, "Vitesse Initiale (m/s):", self.v0_incertitude_var).pack(fill=tk.X, pady=3)
        ...
        # Potentiel
        self.diff_pot_var = tk.StringVar(value=0)
        self.add_labeled_entry(self.base_incertitude_inputs_frame, "Différence de potentiel (V):", self.diff_pot_var).pack(fill=tk.X, pady=3)

        # Potentiel (Slider)
        ttk.Label(self.dynamic_incertitude_inputs_frame, text="Diff. Potentiel Plaque (V):").pack(anchor=tk.W, pady=(5, 0))
        self.slider_frame_v = ttk.Frame(self.dynamic_incertitude_inputs_frame)
        self.slider_frame_v.pack(fill=tk.X, pady=(0, 5))
        self.pot_var = tk.DoubleVar(value=-5000)
        self.pot_slider = ttk.Scale(self.slider_frame_v, from_=-10000, to=10000, orient=tk.HORIZONTAL, variable=self.pot_var, command=self._on_pot_slider_change)
        self.pot_slider.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        ...
        # Bouton Tracer (à placer après le slider ou la case de saisie)
        trace_btn = ttk.Button(self.base_incertitude_inputs_frame, text="Tracer Déviation Électrique", command=self.run_incertitude_simulation)
        trace_btn.pack(pady=15)
        trace_btn = ttk.Button(self.dynamic_incertitude_inputs_frame, text="Tracer Déviation Électrique", command=self.run_incertitude_simulation)
        trace_btn.pack(pady=15)
        
        # Pack enfin la base frame 
        self.base_incertitude_inputs_frame.pack(fill=tk.X, pady=5)
        # --- Bouton pour tracer incertitude xs vs m/q ---
        trace_uncertainty_btn = ttk.Button(frame, text="Tracer incertitude xs vs m/q", command=self.run_incertitude_simulation)
        trace_uncertainty_btn.pack(pady=10)
    ...
    def run_incertitude_simulation(self):
        """
        Trace l'incertitude de xs en fonction de m/q.
        Utilise les valeurs des incertitudes fournies par l'utilisateur.
        """
        if not self.particles_data:
            messagebox.showwarning("Aucune particule", "Veuillez ajouter au moins une particule.")
            self.status_var.set("Ajoutez des particules pour tracer l'incertitude.")
            return

        try:
            # Récupérer les incertitudes relatives
            error_v0 = float(self.error_v0_var.get()) / 100.0
            error_field = float(self.error_field_var.get()) / 100.0

            m_q_values = []
            uncertainty_xs_values = []

            for mass_u, charge_e in self.particles_data:
                # Appel à TA fonction qui calcule l'incertitude sur xs
                # Supposons que tu as une fonction "calculer_incertitude_xs(mass_u, charge_e, error_v0, error_field)"
                delta_xs = calculer_incertitude_xs(mass_u, charge_e, error_v0, error_field)  

                m_q = mass_u / abs(charge_e)  # m/q en u/e
                m_q_values.append(m_q)
                uncertainty_xs_values.append(delta_xs)

            self.ax.cla()
            self.ax.plot(m_q_values, uncertainty_xs_values, 'o-', label="Incertitude xs")
            self.ax.set_xlabel("m/q (u/e)")
            self.ax.set_ylabel("Incertitude xs (m)")
            self.ax.set_title("Incertitude de xs en fonction de m/q")
            self.ax.grid(True)
            self.ax.legend()
            self.canvas.draw()

            self.status_var.set("Tracé incertitude terminé.")
            ...
            print(f"Erreur Tracé Incertitude: {e}")
            self.status_var.set("Erreur tracé incertitude.")
