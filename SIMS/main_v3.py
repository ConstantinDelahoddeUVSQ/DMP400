...
class ParticleApp:
    def __init__(self, root):
        ...
        self.create_pot_widgets(self.pot_tab)   
 ...
 # --- Widgets Électriques ---
    def create_pot_widgets(self, parent):
        """
        Crée les widgets pour la variation de potentiel pour une particule.
        """
        frame = ttk.Frame(parent, padding="10")
        frame.pack(fill=tk.BOTH, expand=True)
        ...
        self.diff_pot_var_ref_pot = tk.StringVar(value="0") # Volts
        self.add_labeled_entry(frame, "Diff. Potentiel Référence (V):", self.diff_pot_var_ref_pot).pack(fill=tk.X, pady=3)

        # --- 2. Checkbox Incertitudes ---
        self.show_uncertainty_var_pot = tk.BooleanVar(value=False)
        # Stocker une référence au checkbox lui-même pour le positionnement
        self.uncertainty_check_pot = ttk.Checkbutton(frame, text="Afficher Incertitudes", variable=self.show_uncertainty_var_pot, command=self.toggle_uncertainty_inputs_pot)
        self.uncertainty_check_pot.pack(anchor=tk.W, padx=5, pady=(5, 0))

        # --- 3. Frame Incertitudes (créé mais packé par toggle) ---
        self.uncertainty_inputs_frame_pot = ttk.LabelFrame(frame, text="Paramètres d'incertitude (%)")
        # Remplir le frame maintenant
        self.delta_v0_percent_var_pot = tk.StringVar(value="1.0")
        self.add_labeled_entry(self.uncertainty_inputs_frame_pot, "ΔV0/V0 (%):", self.delta_v0_percent_var_pot).pack(fill=tk.X, pady=2, padx=5)
        self.delta_theta_percent_var_pot = tk.StringVar(value="1.0")
        self.add_labeled_entry(self.uncertainty_inputs_frame_pot, "Δθ/θ (% de l'angle):", self.delta_theta_percent_var_pot).pack(fill=tk.X, pady=2, padx=5)
        self.delta_h_percent_var_pot = tk.StringVar(value="1.0")
        self.add_labeled_entry(self.uncertainty_inputs_frame_pot, "Δh/h (%):", self.delta_h_percent_var_pot).pack(fill=tk.X, pady=2, padx=5)
        self.delta_E_percent_var_pot = tk.StringVar(value="1.0")
        self.add_labeled_entry(self.uncertainty_inputs_frame_pot, "ΔE/E (%):", self.delta_E_percent_var_pot).pack(fill=tk.X, pady=2, padx=5)
        ttk.Label(self.uncertainty_inputs_frame_pot, text="Note: Δm/m et Δq/q utiliseront des valeurs fixes (0.1%)", font=('Segoe UI', 8)).pack(pady=(5,0))
        # Ne pas packer self.uncertainty_inputs_frame ici

        # --- 4. Séparateur (Stocker une référence) ---
        self.elec_separator_pot = ttk.Separator(frame, orient=tk.HORIZONTAL)
        self.elec_separator_pot.pack(fill=tk.X, pady=10, padx=5)

        # --- 5. Frame Contrôle Mode ---
        # Checkbox pour choisir le mode (Statique/Dynamique) (Stocker une référence)
        self.dynamic_elec_var_pot = tk.BooleanVar(value=False)
        self.dynamic_elec_check_pot = ttk.Checkbutton(frame, text="Mode Dynamique (Sliders)",
                                             variable=self.dynamic_elec_var_pot, command=self.toggle_dynamic_electric_pot)
        self.dynamic_elec_check_pot.pack(anchor=tk.W, padx=5, pady=(5, 0)) # Placé après le séparateur

        # Frames pour widgets statiques et dynamiques (créés mais packés par toggle)
        self.dynamic_electric_inputs_frame_pot = ttk.Frame(frame)
        self.base_electric_inputs_frame_pot = ttk.Frame(frame)

        # --- Widgets Non-Dynamiques (Base) ---
        parent_base_pot = self.base_electric_inputs_frame_pot
        self.v0_elec_var_pot = tk.StringVar(value="1e5") # m/s
        self.add_labeled_entry(parent_base_pot, "Vitesse Initiale (m/s):", self.v0_elec_var_pot).pack(fill=tk.X, pady=3)
        self.diff_pot_var_pot = tk.StringVar(value="-5000") # Volts
        self.add_labeled_entry(parent_base_pot, "Diff. Potentiel (V):", self.diff_pot_var_pot).pack(fill=tk.X, pady=3)
        
        # Bouton pour lancer la simulation statique
        trace_btn_base_pot = ttk.Button(parent_base_pot, text="Tracer Simulation", command=self.run_pot_simulation)
        trace_btn_base_pot.pack(pady=15)

        # --- Widgets Mode Dynamique ---
        parent_dyn_pot = self.dynamic_electric_inputs_frame_pot
        # Limites V0
        self.elec_v0_min_var_pot = tk.StringVar(value="1e4")
        self.add_labeled_entry(parent_dyn_pot, "V0 min (m/s):", self.elec_v0_min_var_pot).pack(fill=tk.X, pady=3)
        self.elec_v0_max_var_pot = tk.StringVar(value="2e5")
        self.add_labeled_entry(parent_dyn_pot, "V0 max (m/s):", self.elec_v0_max_var_pot).pack(fill=tk.X, pady=3)

        # Slider V0
        ttk.Label(parent_dyn_pot, text="Vitesse Initiale V0 (m/s):").pack(anchor=tk.W, pady=(5, 0))
        self.slider_frame_v0_elec_pot = ttk.Frame(parent_dyn_pot)
        self.slider_frame_v0_elec_pot.pack(fill=tk.X, pady=(0, 5))
        self.v0_var_elec_pot = tk.DoubleVar(value=1e5)
        self.v0_slider_elec_pot = ttk.Scale(self.slider_frame_v0_elec_pot, from_=1e4, to=2e5, orient=tk.HORIZONTAL,
                                        variable=self.v0_var_elec_pot, command=self._on_v0_slider_change_elec_pot)
        self.v0_slider_elec_pot.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        self.v0_label_var_elec_pot = tk.StringVar(value=f"{self.v0_var_elec_pot.get():.2e} m/s")
        ttk.Label(self.slider_frame_v0_elec_pot, textvariable=self.v0_label_var_elec_pot, width=12).pack(side=tk.LEFT)

        # Limites Potentiel
        self.diff_pot_min_var_pot = tk.StringVar(value="-10000")
        self.add_labeled_entry(parent_dyn_pot, "Potentiel min (V):", self.diff_pot_min_var_pot).pack(fill=tk.X, pady=3)
        self.diff_pot_max_var_pot = tk.StringVar(value="10000")
        self.add_labeled_entry(parent_dyn_pot, "Potentiel max (V):", self.diff_pot_max_var_pot).pack(fill=tk.X, pady=3)

        # Slider Potentiel
        ttk.Label(parent_dyn_pot, text="Diff. Potentiel (V):").pack(anchor=tk.W, pady=(5, 0))
        self.slider_frame_v_pot = ttk.Frame(parent_dyn_pot)
        self.slider_frame_v_pot.pack(fill=tk.X, pady=(0, 5))
        self.pot_var_pot = tk.DoubleVar(value=-5000)
        self.pot_slider_pot = ttk.Scale(self.slider_frame_v_pot, from_=-10000, to=10000, orient=tk.HORIZONTAL,
                                    variable=self.pot_var_pot, command=self._on_pot_slider_change_pot)
        self.pot_slider_pot.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        self.pot_label_var_pot = tk.StringVar(value=f"{self.pot_var_pot.get():.1f} V")
        ttk.Label(self.slider_frame_v_pot, textvariable=self.pot_label_var_pot, width=12).pack(side=tk.LEFT)

        # Bouton pour appliquer les limites et tracer initialement
        apply_limits_btn_dyn_pot = ttk.Button(parent_dyn_pot, text="Appliquer Limites & Tracer", command=self.run_pot_simulation)
        apply_limits_btn_dyn_pot.pack(pady=15)

        # --- Afficher/Cacher initialement ---
        # IMPORTANT: Appeler les toggles APRÈS que les widgets de référence (separator, checkbutton) soient packés
        self.toggle_uncertainty_inputs_pot() # Appel initial pour cacher/montrer le frame incertitude
        self.toggle_dynamic_electric_pot() # Appel initial pour packer le bon frame (statique/dynamique)

    def toggle_dynamic_electric_pot(self):
        """Gère l'affichage des widgets électriques dynamique/statique."""
        if self.dynamic_elec_var_pot.get():
            self.base_electric_inputs_frame_pot.pack_forget()
            # Pack le frame dynamique APRÈS le checkbox correspondant
            self.dynamic_electric_inputs_frame_pot.pack(fill=tk.X, pady=5, padx=5, after=self.dynamic_elec_check_pot)
        else:
            self.dynamic_electric_inputs_frame_pot.pack_forget()
            # Pack le frame statique APRÈS le checkbox correspondant
            self.base_electric_inputs_frame_pot.pack(fill=tk.X, pady=5, padx=5, after=self.dynamic_elec_check_pot)
        # Mettre à jour la scrollregion car la hauteur change
        self.root.after(50, self._update_scroll_region_and_bar)

    def toggle_uncertainty_inputs_pot(self):
        """Affiche ou cache le frame des entrées d'incertitude."""
        if self.show_uncertainty_var_pot.get():
            # Pack le frame incertitude AVANT le séparateur
            self.uncertainty_inputs_frame_pot.pack(fill=tk.X, pady=(5,0), padx=5, before=self.elec_separator_pot)
        else:
    ...
    # --- Callbacks Sliders Électriques ---
    def _on_pot_slider_change_pot(self, event=None):
        self._update_pot_label_pot()
        if self.particles_data:
            self.run_pot_simulation(called_by_slider=True)

    def _on_v0_slider_change_elec_pot(self, event=None) :
        self._update_v0_label_elec_pot()
        if self.particles_data:
            self.run_pot_simulation(called_by_slider=True)

    def _update_pot_label_pot(self, event=None):
        self.pot_label_var_pot.set(f"{self.pot_var_pot.get():.1f} V")

    def _update_v0_label_elec_pot(self, event=None):
        self.v0_label_var_elec_pot.set(f"{self.v0_var_elec_pot.get():.2e} m/s")
    ...
    def run_pot_simulation(self, called_by_slider=False):
        """Lance la simulation de déviation électrique en utilisant le nouveau module deviation.py."""
        if not self.particles_data:
            if not called_by_slider: messagebox.showwarning("Aucune Particule", "Veuillez ajouter au moins une particule.", parent=self.root)
            self.status_var.set("Ajoutez des particules pour simuler.")
        ...
        if len(self.particles_data) > 1 :
            if not called_by_slider: messagebox.showwarning("Trop de particules", "Veuillez ajouter une seule particule.", parent=self.root)
            self.status_var.set("Retirez des particules pour simuler.")
            self.ax.cla()
            self.ax.set_title("Déviation Électrique")
            self.ax.set_xlabel("x (m)")
            self.ax.set_ylabel("y (m)")
        ...
        try:
            ...
            ref_pot = self.diff_pot_var_ref_pot.get().strip().replace(',', '.')
            if not angle_deg_str or not dist_str or not ref_pot: raise ValueError("Angle et Distance/Hauteur requis.")
            angle_deg = float(angle_deg_str)
            hauteur_distance = float(dist_str)
            ref_pot = float(ref_pot)
            ...
            # Lire V0 et Potentiel selon le mode
            if not self.dynamic_elec_var_pot.get(): 
                # Mode Statique
                v0_str = self.v0_elec_var_pot.get().strip().replace(',', '.')
                pot_str = self.diff_pot_var_pot.get().strip().replace(',', '.')
                if not v0_str or not pot_str: raise ValueError("V0 et Potentiel requis.")
                v0 = float(v0_str)
                potentiel = float(pot_str)
                if v0 <= 0 : raise ValueError("Vitesse initiale doit être > 0.")
                ...
                if not called_by_slider :
                    # Mise à jour limites/valeurs
                    pot_min_str = self.diff_pot_min_var_pot.get().strip().replace(',', '.')
                    pot_max_str = self.diff_pot_max_var_pot.get().strip().replace(',', '.')
                    v0_min_str = self.elec_v0_min_var_pot.get().strip().replace(',', '.')
                    v0_max_str = self.elec_v0_max_var_pot.get().strip().replace(',', '.')
                    if not all([pot_min_str, pot_max_str, v0_min_str, v0_max_str]):
                         raise ValueError("Les limites min/max ne peuvent être vides.")
                    ...
                    current_pot = self.pot_var_pot.get()
                    current_v0 = self.v0_var_elec_pot.get()

                    self.pot_slider_pot.config(from_=pot_min, to=pot_max)
                    if not (pot_min <= current_pot <= pot_max):
                        pot_init = (pot_max + pot_min) / 2
                        self.pot_var_pot.set(pot_init)
                    self._update_pot_label_pot()

                    self.v0_slider_elec_pot.config(from_=v0_min, to=v0_max)
                    if not (v0_min <= current_v0 <= v0_max):
                        v0_init = (v0_max + v0_min) / 2
                        self.v0_var_elec_pot.set(v0_init)
                    self._update_v0_label_elec_pot()

                v0 = self.v0_var_elec_pot.get()
                potentiel = self.pot_var_pot.get()
            ...
            # --- Effacer et Lancer Simulation ---
            self.ax.cla()
            show_uncertainty = self.show_uncertainty_var_pot.get()
            self.status_var.set(f"Calcul déviation électrique {'avec' if show_uncertainty else 'sans'} incertitude...")
            self.root.update_idletasks()
            ...
            if show_uncertainty:
                ...
                try:
                    incertitudes_dict = {
                        'v0': float(self.delta_v0_percent_var_pot.get().strip().replace(',', '.')) / 100.0,
                        'theta': float(self.delta_theta_percent_var_pot.get().strip().replace(',', '.')) / 100.0, # % de l'angle
                        'h': float(self.delta_h_percent_var_pot.get().strip().replace(',', '.')) / 100.0, # % hauteur/distance
                        'E': float(self.delta_E_percent_var_pot.get().strip().replace(',', '.')) / 100.0, # % Champ E
                        'm': 0.001, # 0.1% fixe
                        'q': 0.0001 # 0.01% fixe
                    }
                    pots = [ref_pot, potentiel]
                    # Appeler la fonction avec incertitudes
                    deviation.tracer_ensemble_trajectoires_avec_incertitudes(
                        masse_charge_list,
                        vitesse_initiale=v0,
