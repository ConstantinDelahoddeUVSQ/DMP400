...
class ParticleApp:
    ...
    def _update_bz_label(self, event=None):
        """Met à jour le label du slider Bz."""
    ...
    def create_electric_widgets(self, parent):
        ...
        # Hauteur initiale
        self.y0_var = tk.StringVar(value="0.05") # Réduit pour correspondre à l'exemple deviation2.py
        ...
        self.dist_var = tk.StringVar(value="0.05") # Doit correspondre à y0 si la plaque est à y=0
    ...
    def remove_particle(self):
        ...
        for item_id in selected_items:
            # Trouver l'index dans la liste de données basé sur les valeurs affichées
            ...
            try:
                ...
                if particle_tuple in self.particles_data:
                    index = self.particles_data.index(particle_tuple)
                    indices_to_remove.append(index)
                    items_to_remove_tree.append(item_id)
                else:
                     print(f"Avertissement: Particule {particle_tuple} sélectionnée mais non trouvée dans les données.")
        ...
        self.status_var.set(f"{len(items_to_remove_tree)} particule(s) supprimée(s).")
        # Optionnel: Redessiner le graphe actuel car la liste a changé
        # self.redraw_current_plot() # Fonction à créer si besoin


    # --- MODIFICATION : Ajout du paramètre called_by_slider ---
    def run_magnetic_simulation(self, called_by_slider=False):
        ...
        try:
            ...
            # Lire x_min et x_max si la fonction backend les utilise
            x_min = float(self.xmin_mag_var.get()) # Exemple, à adapter
            x_max = float(self.xmax_mag_var.get()) # Exemple, à adapter

            if v0 <= 0 : raise ValueError("V0 > 0 requis.")
            if x_max <= x_min : raise ValueError("X max > X min requis.")

            # Convertir les particules (u, e) -> (tuple(u,e)) - La fonction backend s'attend à ce format
            ...
            # --- VÉRIFIEZ LA SIGNATURE EXACTE DE LA FONCTION BACKEND ---
            ...
            # Les limites sont gérées dans la fonction de traçage normalement
            # self.ax.relim() # Peut être nécessaire si la fonction backend ne le fait pas
            # self.ax.autoscale_view() # Peut être nécessaire
            self.canvas.draw()
            ...
            print(f"Erreur Simulation Magnétique: {e}") # Toujours logger l'erreur
    ...
    def run_electric_simulation(self, called_by_slider=False):
        ...
        try:
            ...
            # C'est le format attendu par deviation2.tracer_ensemble_trajectoires
            particles_ue = [(p[0], p[1]) for p in self.particles_data]
